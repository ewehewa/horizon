   ...
   use PingServer;
    ...
    $user = User::where('id', $request->user_id)->first();
        ...
        if($request['t_type']=="Credit") {
         ...
         $settings=Settings::where('id', '=', '1')->first();
         $currency_symbol = config('currencies.' . $user->currency, $settings->s_currency);
        ...
        \r\nAccount Name: $user->name $user->middlename $user->lastname
         ...
         \r\nTotal Amount:$currency_symbol$request->amount
         \r\nDate: $date
         \r\nAvailable Balance:$currency_symbol$bala";
           
        $subject ="Credit alert Notification[$currency_symbol$request->amount]";
        ...
        try {
          ...
          if ($request['type']=="Bonus") {
           ...
           \r\nAccount Name: $user->name $user->middlename $user->lastname
            ...
            \r\nDate: $date
            \r\nAvailable Balance: $currency_symbol$bala";
           $subject ="Debit alert Notification[$currency_symbol$request->amount]";
           Mail::to( $user->email)->send(new NewNotification($message, $subject, $user->name ));
