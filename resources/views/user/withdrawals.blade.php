						...
						<div class="card-deck">
							<div
								...
								<div class="py-0 border-0 card-header">
									<span
										class="px-4 py-1 mx-auto bg-white shadow-sm h6 d-inline-block rounded-bottom">{{
										$method->name }}</span>
									<div class="py-5">
										<img src="{{ asset('dash2/img/Wallet.svg.png') }}" alt="withdrawal method image"
											srcset="" class="img-fluid img-center" style="height:90px;">
								...
								<div class="card-body">
									<ul class="mb-4 text-white list-unstyled">
										<li>
											...
											<p class="text-white h5">{{ config('currencies.'.Auth::user()->currency,
												'$') }}{{ number_format($method->minimum) }}</p>
										</li>
										<li>
											<small>Maximum withdrawable amount</small>
											<p class="text-white h5">{{ config('currencies.'.Auth::user()->currency,
												'$') }}{{ number_format($method->maximum )}}</p>
										</li>
