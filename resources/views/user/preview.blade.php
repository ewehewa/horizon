                ...
                <div class="relative bg-gradient-to-r from-primary-600 to-primary-700 px-6 py-8 text-white">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <img class="h-12 w-auto" src="{{ asset('storage/app/public/' . $settings->logo) }}"
                ...
                <div class="p-6 md:p-8">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-gray-600 text-sm mb-4">
                            ...
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->accountname }}.
                            @elseif($dp->payment_mode == 'Domestic Transfer')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->accountname }}.
                            @elseif($dp->payment_mode == 'Cryptocurrency')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to the {{ $dp->crypto_currency }} wallet.
                            @elseif($dp->payment_mode == 'PayPal')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to PayPal.
                            @elseif($dp->payment_mode == 'Wise Transfer')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->wise_fullname }} via Wise.
                            @elseif($dp->payment_mode == 'Skrill')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->skrill_fullname }} via Skrill.
                            @elseif($dp->payment_mode == 'Venmo')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->venmo_username }} via Venmo.
                            @elseif($dp->payment_mode == 'Zelle')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->zelle_name }} via Zelle.
                            @elseif($dp->payment_mode == 'Cash App')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->cash_app_tag }} via Cash App.
                            @elseif($dp->payment_mode == 'Revolut')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->revolut_fullname }} via Revolut.
                            @elseif($dp->payment_mode == 'Alipay')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->alipay_fullname }} via Alipay.
                            @elseif($dp->payment_mode == 'WeChat Pay')
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }} to {{ $dp->wechat_name }} via WeChat Pay.
                            @else
                            You have successfully transferred {{ config('currencies.'.Auth::user()->currency, '$') }}{{
                            $dp->amount }}.
                            @endif
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <p class="text-sm font-medium text-gray-900">{{
                                            \Carbon\Carbon::parse($dp->date)->format('M d, Y') }}</p>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <p class="text-sm font-medium text-gray-900">{{
                                            \Carbon\Carbon::parse($dp->date)->format('h:i A') }}</p>
                                    </div>
                                </div>
                                <div>
                        ...
                        <div class="bg-white rounded-lg border border-gray-200 p-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <p class="text-sm font-medium text-gray-900 break-all">{{ $dp->wallet_address ??
                                        'N/A' }}</p>
                                </div>
                            </div>
                            @elseif($dp->payment_mode == 'PayPal')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <p class="text-sm text-gray-500 mb-1">Full Name</p>
                                    <p class="text-sm font-medium text-gray-900">{{ $dp->cash_app_fullname ?? 'N/A' }}
                                    </p>
                                </div>
                            </div>
                            @elseif($dp->payment_mode == 'Revolut')
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <p class="text-sm text-gray-500 mb-1">Full Name</p>
                                    <p class="text-sm font-medium text-gray-900">{{ $dp->revolut_fullname ?? 'N/A' }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 mb-1">Email</p>
                                ...
                                <div class="grid grid-cols-2 gap-4 p-4">
                                    <div>
                                        ...
                                        <p class="text-base font-medium text-gray-900">{{
                                            config('currencies.'.Auth::user()->currency, '$') }}{{ $dp->amount }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-500">Handling & Charges</p>
                                        <p class="text-base font-medium text-gray-900">{{
                                            config('currencies.'.Auth::user()->currency, '$') }}0</p>
                                    </div>
                                </div>
                                <div class="p-4 bg-gray-50">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <p class="text-lg font-bold text-primary-700">{{
                                            config('currencies.'.Auth::user()->currency, '$') }}{{ $dp->bal }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 text-center border-t border-gray-100 pt-6">
                        ...
                        <p class="text-xs text-gray-500 mb-1">This receipt serves as confirmation of this transaction.
                        </p>
                        <p class="text-xs text-gray-500">For any issues or inquiries regarding this transaction, please
                            contact support.</p>
                        <p class="text-xs text-gray-400 mt-1">© {{ date('Y') }} {{ $settings->site_name }}. All rights
                            reserved.</p>
                    </div>
                </div>
            </div>
